...
| R6 | `Run tests` / `STATUS_STACK_BUFFER_OVERRUN` crash in voice-pack-analyze subprocess | Voice-pack-analyze loads CUDA models on the CI runner which has no GPU; the native stack overflows | Ensure the chunked orchestrator (`src/voice_pack_chunked_subproc.py`) passes `--device cpu` when `CUDA_VISIBLE_DEVICES` is unset or empty. The fix is a one-line guard at the subprocess launch site. Commit `fix(ci): force CPU fallback in chunked analyzer when no GPU is available`. |

If none of the six recipes match, go to Phase 4 before writing new code.
