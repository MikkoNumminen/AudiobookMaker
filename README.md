...
- **Finnish text normalizer** with 21 normalization passes covering `-ismi` / `-tio` stems, abbreviations, ordinals, Latin phrases, Roman numerals, compound-word seam splitting, colon-suffixed numerals (`20:een`), colon ratios (`1:5` → "yksi viiteen"), case clitics on acronyms (`README:en`), space-separated thousands (`3 755 242`), maths symbols, and acronym handling. 330+ unit tests for this module alone. The reason this is here at all is that Finnish TTS pronunciation gets weird in predictable ways, and the normalizer is what makes the difference between "robotic" and "audiobook-grade".
