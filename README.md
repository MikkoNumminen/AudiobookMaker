...
- English US voices: Jenny, Aria, Ava, Guy, Andrew, and more
- English GB voices: Sonia, Ryan, and more
...
- **Finnish text normalizer** with 19 normalization passes covering `-ismi` / `-tio` stems, abbreviations, ordinals, Latin phrases, Roman numerals, compound-word seam splitting, colon-suffixed numerals (`20:een`), space-separated thousands (`3 755 242`), maths symbols, and acronym handling. 330+ unit tests for this module alone. The reason this is here at all is that Finnish TTS pronunciation gets weird in predictable ways, and the normalizer is what makes the difference between "robotic" and "audiobook-grade".
