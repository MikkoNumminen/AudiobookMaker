...
The interesting one. Chatterbox is an open-source neural TTS from Resemble AI; `Finnish-NLP/Chatterbox-Finnish` is a Finnish fine-tune by the Finnish-NLP organization. Together they produce the best Finnish voice quality in this app — close to commercial audiobook quality on prepared text — and the engine supports voice imitation from a short reference clip at synthesis time (no training step).
...
- **Voice imitation from a short reference audio clip** — drop in a sample and the engine narrates in that voice. Inference-time only; no training step, no voice pack produced.
...
For **Chatterbox** (Grandmom voice, imported voice packs, reference-clip imitation), open the GUI and click **Install engines…** in the Settings panel. The app downloads the Chatterbox venv + Finnish-NLP model on demand (one-time ~15 GB). After that it works fully offline.
...
- **Reference-clip quality matters.** A noisy reference produces a noisy result — true for both the dev-only voice-cloning pipeline (which trains a voice pack from a recording) and the GUI's reference-clip imitation (Chatterbox / VoxCPM2, which uses the clip at synthesis time). The audio preflight check exists for a reason; don't bypass it.
