...
- [ ] Pass H morpheme-splitter false positive: inside compound numbers like `kaksisataaneljäkymmentäkaksi` (242), the regex prefers the longer illative form `sataan` over partitive `sataa` because longest-first ordering wins, producing `kaksisataan eljäkymmentä kaksi` instead of the correct `kaksisataa neljäkymmentä kaksi`. Fix: reorder the alternation so partitive `sataa`/`kymmentä` match first, and make the illative forms require a specific lookbehind (e.g. only after `sivu`/`sivuun` context, not inside a compound number). 🟢 🧠 Opus
- [ ] `§` (section symbol) expansion: `§ 242` should become `pykälä 242` so Pass G can apply the `pykälä`/`pykälässä` governor. Add to Pass M unit expansion. 🟢 ⚡ Sonnet
