...
- [ ] Source PDF: `2004_Pihlajamäki_Johdatus.pdf`. Output: `TURO_00_full.mp3.mpeg`. Turo reported sentence endings get swallowed/cut off after long listening; he first called out ~4:55–5:10 as especially bad, then clarified the issue is **worse after that** — likely **the whole tail of the book** from roughly that point onward, not only a short window. Likely root causes: (a) chunk boundary cuts mid-sentence and the TTS trails off, (b) silence trimming (`_trim_chunk_silence`) is too aggressive and clips the tail, (c) Chatterbox model artifact on long sequences where it forces EOS early, (d) cumulative drift or resource/state issues late in a long run. Reproduce from the output file + PDF text alignment, then fix. 🟡 🧠 Opus.
