...
The rule is **substring-strict**: backticks, code fences, technical
context, and "but it's a tool name" do **not** immunise. If the token
is present in the text, the rule is broken. The intent behind the rule
is preventing authorship attribution, but enforcement is by literal
substring — that absoluteness is what makes the rule operable, and
softening it has historically led to slips.

The only narrow exception is this very policy document quoting its
own forbidden tokens to define what is forbidden. Text outside of
CLAUDE.md has no licence to do the same.

Practical consequence: when a commit body or PR description would
naturally reference a skill name like `ai-codegen-smell-audit` or an
SDK tool name like `Agent`, **rephrase** to avoid the substring.
Examples:

- "the smell-audit skill" instead of `ai-codegen-smell-audit`
- "tool dispatch" / "subordinate run" instead of `Agent` / `subagent`
- "the audit pass" / "this round" instead of "this session" or
  "this run"

The friction of rephrasing is small; the clarity of an absolute
substring rule is worth keeping. The auto-classifier defaults to
false-positive blocking, which is the right safety bias.
