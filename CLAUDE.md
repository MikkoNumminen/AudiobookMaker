...
The repo's public history must not reveal that automation produced the
changes. The forbidden tokens are `Claude`, `Anthropic`, `AI`, `agent`,
`session`, `Co-Authored-By`, and any variant. Never put these in a
commit subject, body, trailer, tag, or PR description **as authorship
attribution**.

The test: would a reader infer "this was written by an AI"? If yes,
forbidden. If no — just a topic word in normal English use, or a
codebase identifier in backticks — allowed.

**Forbidden** (anything implying automation produced the code):

- `Co-Authored-By: Claude` / `Co-Authored-By: …@anthropic.com`
- "generated by Claude", "AI-assisted change", "AI-origin commit"
- "the agent made this change", "in this session", "agent run"
- "auto-generated by Claude Code" / "produced by Anthropic's tool"

**Allowed** (technical identifiers in their normal sense, quoted in
backticks so context is unambiguous):

- `ai-codegen-smell-audit`, `audit-followup`, or any skill slug that
  happens to contain a forbidden token in its name
- `Agent` / `Explore` as Anthropic SDK tool names in technical
  references (e.g., "added `Agent` dispatch", "the `Explore` subagent
  returned no findings")
- "audit session" / "review session" / "the audit pass" when "session"
  / "pass" plainly means an audit run, not a human-vs-AI marker
- `Co-Authored-By` discussed inside this policy document
- Quoting a forbidden token from a third-party error message or
  upstream library docstring

Before every `git commit`:

1. Draft the message.
2. Re-read slowly, applying the test above.
3. If in doubt, rephrase to avoid the token entirely. The auto-
   classifier defaults to false-positive blocking, which is the
   right safety bias — accept the friction over the slip.
...
This rule has been broken before; every authorship-attribution
violation requires a force-push to scrub.
