...
|---|---|---|---|
| pyannote speaker diarization | Tells "who spoke when" across a long audio source; default backend for the dev-only `scripts/voice_pack_analyze.py` script. (The in-app voice-cloning surface was removed in v3.12.0.) | `HF_TOKEN` env var + accepted license on the gated repo. | [huggingface.co/pyannote/speaker-diarization-3.1](https://huggingface.co/pyannote/speaker-diarization-3.1) |
